...
import cn.yang.galleryfinal.utils.MediaScanner;
...
public abstract class PhotoBaseActivity extends Activity implements EasyPermissions.PermissionCallbacks {
    ...
    protected void resultFailureDelayed(String errormsg, boolean delayFinish) {
        GalleryFinal.OnMediaResultCallback callback = GalleryFinal.getCallback();
    ...
    protected void resultData(ArrayList<PhotoInfo> photoList) {
        ...
        if (callback != null) {
            if (photoList != null && photoList.size() > 0) {
                callback.onHandlerSuccess(requestCode, filterPhoto(photoList));
     ...
     * 过滤旋转的照片
     */
    private List<PhotoInfo> filterPhoto(ArrayList<PhotoInfo> photoList) {
        ArrayList<PhotoInfo> pl = new ArrayList<>();
        for (PhotoInfo p : photoList) {
            //如果图片被旋转，要把宽高也互换
            if (Utils.getBitmapDegree(p.getPhotoPath()) != 0) {
                int temp = p.getWidth();
                p.setWidth(p.getHeight());
                p.setHeight(temp);
            }
            p.setPhotoPath(Utils.readPictureDegree(p.getPhotoPath()));
            pl.add(p);
        }
        return pl;
    }
